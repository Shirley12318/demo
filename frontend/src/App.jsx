...
function App() {
  ...
  const [identity, setIdentity] = useState("普通学生");
  ...
  useEffect(() => {
    ...
    const loadDailyNews = async () => {
      ...
      try {
        const res = await fetch('http://127.0.0.1:8000/news/daily?limit=8');
  ...
  useEffect(() => {
    ...
    const loadRecommendedNews = async () => {
      ...
      try {
        const res = await fetch(`http://127.0.0.1:8000/news/recommended?username=${encodeURIComponent(username)}&limit=6`);
        const data = await res.json();
  ...
  useEffect(() => {
    ...
    const loadDiscussionCatalog = async () => {
      try {
        const res = await fetch('http://127.0.0.1:8000/discussion/topics');
        const data = await res.json();
  ...
  useEffect(() => {
    ...
    const loadDiscussionPosts = async () => {
      ...
      try {
        const res = await fetch(`http://127.0.0.1:8000/discussion/posts?topic=${encodeURIComponent(selectedDiscussionTopic)}&username=${encodeURIComponent(username)}&limit=50`);
        const data = await res.json();
  ...
  const loadConversationMessages = async (conversationId, activeUsername = username) => {
    ...
    const res = await fetch(
      `http://127.0.0.1:8000/conversations/${conversationId}/messages?username=${encodeURIComponent(activeUsername)}`,
      { method: 'GET' }
    );
    const data = await res.json();
  ...
  const loadConversations = async (activeUsername = username) => {
    const res = await fetch(`http://127.0.0.1:8000/conversations?username=${encodeURIComponent(activeUsername)}`, {
      method: 'GET'
    });
    const data = await res.json();
  ...
  const handlePublishDiscussionPost = async () => {
    ...
    try {
      const res = await fetch('http://127.0.0.1:8000/discussion/posts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          username,
  ...
  const handleToggleDiscussionLike = async (postId) => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8000/discussion/posts/${postId}/like`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleReportDiscussionPost = async (postId) => {
    setDiscussionActionPostId(postId);
    try {
      const res = await fetch(`http://127.0.0.1:8000/discussion/posts/${postId}/report`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username })
      });
  ...
  const handleSend = async () => {
    ...
    const userMsgIndex = messages.length; // 记录当前用户消息的索引
    setInput(""); // 清空输入框
    setIsLoading(true);
    ...
    try {
      ...
      }, 10); // 每20ms显示一个字（可改为10-50之间的值调整速度）
      
      if (data.portrait_analysis) {
        // 更新用户消息的分析结果，添加chatId用于数据库查询
        setMessages(prev => {
          const updated = [...prev];
          if (updated[userMsgIndex]) {
            updated[userMsgIndex] = {
              ...updated[userMsgIndex],
              chatId: data.chat_id,  // 保存chat_id用于后续数据库查询
              analysis: {
                  ...
                  <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`dialog-block ${m.role === 'user' ? 'user' : ''}`} onClick={async () => {
                      if (m.role === 'ai' && m.aiAnalysis) {
                        if (m.chatId) {
                          try {
                            const res = await fetch(`http://127.0.0.1:8000/chat/history/${m.chatId}`);
                            if (res.ok) {
                              const data = await res.json();
              ...
              {!isDiscussionLoading && !discussionError && discussionPosts.map((post, index) => (
                <article key={post.id} className="post-card">
                  <div className="post-head">
                    <div className="post-avatar" style={{ background: [
                    ...
                    ][index % 4] }}>{(post.author || '用').charAt(0)}</div>
