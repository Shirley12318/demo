...
// 从环境变量读取后端基础地址，如果没有则使用 Railway 线上地址
...
function DataOverview({ onBack, username }) {
    ...
    const fetchOverviewData = async () => {
        ...
        try {
            setLoading(true);
            setError('');
            // 修改点：使用 API_BASE_URL 替代硬编码的 http://127.0.0.1:8000
            const res = await fetch(`${API_BASE_URL}/api/overview?username=${encodeURIComponent(username)}`);
                                ...
                                riskMapping.map((risk, idx) => (
                                    <div key={idx} className="p-3 bg-slate-50 rounded border-l-2 border-red-300">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span className={`text-xs px-2 py-1 rounded ${risk.level === 'high' ? 'bg-red-100 text-red-700' :
...
function StatCard({ icon: Icon, label, value, color }) {
    return (
        <div className={`bg-white rounded-lg shadow-md p-4 border-l-4 ${color === 'bg-blue-50 text-blue-600' ? 'border-blue-500' :
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className={`text-3xl font-bold ${color === 'bg-blue-50 text-blue-600' ? 'text-blue-600' :
